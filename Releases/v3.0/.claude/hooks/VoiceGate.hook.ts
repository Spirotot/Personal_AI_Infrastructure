 ...
 * 2. Voice globally disabled (settings.json or PAI_VOICE_ENABLED=false) → PASS (curl fails silently; keeps PostToolUse alive for phase tracking)
...
async function main() {
  ...
  // Voice globally disabled → let curl through; with -s flag a refused connection
  // is completely silent (no output, non-zero exit ignored). This keeps
  // AlgorithmTracker's PostToolUse firing so phase detection stays intact.
  if (!isVoiceEnabled()) {
    console.log(JSON.stringify({ continue: true }));
    return;
  }
