...
[ISCPersist.hook.ts (PostToolUse/TaskCreate+TaskUpdate) automatically syncs ISC criteria to MEMORY/WORK/{session}/ISC.json — no manual action required]
...
[ISCPersist.hook.ts automatically writes final satisfaction stats to WORK/ISC.json on each TaskUpdate — satisfaction reflects completion state at VERIFY time]
